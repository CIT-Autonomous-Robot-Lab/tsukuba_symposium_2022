...
\end{itemize}
したからである。
実際にオドメトリ誤差に関するパラメータを動的に変更するための実装を行った.\cite{githubのリンク} 
これは, ある場所において動的にパラメータを変更することで, 安定した自己位置推定することを狙った実装である.
...
順調に2DLiDARのみによる自己位置推定を行うことができた。
人混みや待機列に遭遇した場合は2D LiDARのほぼすべての情報が未知障害物であり、地図との照合ができず
自己位置推定が破綻することがあった. 
こうした状況で真に近い自己位置を推定するには, ホイールオドメトリのみに頼ることや他のセンサを追加で使用する必要
があると考える. 特に信号あり横断歩道の停止位置では、周りに多くの未知障害物や環境の段差が存在しても数十cm単位で
自己位置を推定することが求められるので, IMUやGNSSなどのセンサを追加することが必要だと考えられる. 
