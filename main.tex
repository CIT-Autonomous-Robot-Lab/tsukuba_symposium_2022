...
スタートからの直線は、ランドマークとなる植木やプランターが
他のロボットや歩行者（観覧者）によって観測不能隣となり、自己位置が不確かな場面が複数回あった。
信号あり横断歩道の停止線前の20m, 市役所側の線路隣の路地, 公園側の線路隣の路地は, 
ロボットは移動していたが, 自己位置推定のパーティクルが移動しない問題があった. 
信号あり横断歩道往路（2回目の停止位置での停止）は, 停止エリアに点字ブロックが存在し、
さらに2D LiDAR前方のほとんどのセンサ情報が地面になってしまうほどの勾配になっており, 
その影響で自己位置推定が不確かであった.
信号あり横断歩道往路(横断歩道の横断)は, 点字ブロックに足を大きく持っていかれ, 回転方向に大きく自己位置推定が
ずれることがあった. 
...
% ↓　Bチームの独自の試みとして重りを載せたことを言いたかったので書きました。
また、実験走行においてレンガの間に生えている雑草上を走行した際、車輪のスリップにより、
回転方向に自己位置推定がずれることがあった。
そのため、Bチームでは機体の安定性向上を期待し2kgの重りを搭載したが、その実際の効果については今後検証が必要である。
